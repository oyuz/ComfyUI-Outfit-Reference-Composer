...
- 上装、下装、袜子和鞋按语义间距紧凑排列；默认互不遮挡、单品完整可见。`length: "floor"` 的长裤默认遮住鞋面；短裤和裙子仍让鞋／靴完整露出。原始单品图不变。
- 项链最后绘制，是唯一允许覆盖上装的类别。
...
| 下装 | `shoe_coverage` | 可选：`none` 取消某件及地长裤的默认遮鞋；`cover_most` 与默认效果相同 |
| 袜子 | `length` | `ankle`, `crew`, `mid_calf`, `knee`, `knee_high` |
| 鞋 | `footwear_type` | `shoes`（默认）, `boots`（靴筒也由长裤遮住） |
| 配饰 | `size` | `tiny`, `small`, `medium`, `large` |

手镯不写位置时默认放在画面左侧；可用 `"side": "right"` 或 `"placement": "right_wrist"` 明确放到右侧。包保持在画面右侧手持区域。

现在直接以 `length: "floor"` 为判断标准，不必再把“穿着时覆盖大部分鞋面”的中文句子传入节点。只有 `garment_type: "pants"`（旧 JSON 未写类型时默认为非裙装）且显式写了 `length: "floor"` 的长裤才触发：普通鞋约露出下半部分，鞋子写 `"footwear_type": "boots"` 时只露底部鞋身、靴筒被遮住。短裤、裙子、非 `floor` 下装仍让整双鞋／靴完整显示。节点不会自行读取飞书文档；靴子的类型需在鞋的 JSON 中注明，或随 JSON 传入含“靴”的鞋款描述。示例：
...
- 上装、下装、袜子和鞋先完成等比测量，再依次紧凑排版。露脐组合保留较大的腹部语义间距；短裙/短裤到鞋之间保留有限的腿部语义间距。及地长裤的鞋面遮挡默认启用：鞋的隐藏部分放在裤脚后方，输出时遮去这部分以免靴筒从裤腿两侧漏出；鞋本身仍等比缩放。
...
- 所有单品仍受画布与横向安全区限制，节点不会用裁切来隐藏越界部分；唯一例外是明确要求由长裤遮住的鞋／靴上部。
